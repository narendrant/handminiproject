...
$keyword=$_POST['keyword'];
    ...
    $query= "SELECT pname,category,product_ID FROM Product WHERE location = ? AND pname LIKE ? ";
    $loc=$_POST['loc'];
		$stmt=$conn->prepare($query);
    //echo "Prepare failed: (" . $conn->errno . ") " . $conn->error;
    $param="%".$keyword."%";		
    $stmt->bind_param('ss',$loc,$param);	
		$stmt->execute();
		$category='';
      ...
      $result=$result."<a href='product.php?pid=".$pid."'  ><li  class='bold' style='float:left;text-align:justify; padding-left:20px;'>".$pname."</li></a><br><li class='divider'></li><br>";
   	 ...
   	 {
   	$query="SELECT pname,category,product_ID FROM Product WHERE pname LIKE ?";
		$stmt=$conn->prepare($query);
    $param="%".$keyword."%";
